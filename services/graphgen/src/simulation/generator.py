...
from ..kg.config.settings import LLMSettings

def get_model_name():
    # Use centralized configuration
    return LLMSettings().base_model
...
def scenario_director_node(state: GraphState) -> GraphState:
    ...
    if current_time is None:
        ...
        if state['retry_count'] == 0:
            ...
            # ERROR FIX: Increase interval to reduce total calls (was 45-150)
            jump_minutes = random.randint(120, 240) # 2 to 4 hours steps
            current_time += timedelta(minutes=jump_minutes)
    
    # Skip nighttime (23:00 - 06:30)
    if current_time.hour >= 22 or current_time.hour < 7: # End day slightly earlier
...
def quality_validator_node(state: GraphState) -> GraphState:
    ...
    # --- Audio Validation & Auto-fix ---
    # Must have Speaker N: format
    if not re.search(r"Speaker \d+:", audio):
        logger.warning(f"⚠️ Audio missing 'Speaker N:' format. Force-fixing.")
        audio = "Speaker 1:\n" + audio
    
    # Remove stage directions in dialogue
    if re.search(r"Speaker \d+:.*\((pauses|sighs|walks|types|laughs|thinking|looks)\)", audio[:500]):
        logger.warning(f"⚠️ Audio contains stage directions. Cleaning.")
        audio = re.sub(r"\((pauses|sighs|walks|types|laughs|thinking|looks.*?)\)", "", audio, flags=re.IGNORECASE)
    
    # --- Image Validation & Auto-fix ---
    # No first person - try to replace generic terms but mostly just warn and accept
    if re.search(r"\b(I|my|me|mine)\b", image, re.IGNORECASE):
        logger.warning(f"⚠️ Image description used first person. Accepting anyway.")
    
    # No camera references
    if re.search(r"\b(camera|captures?|scene shows|we see|viewer)\b", image, re.IGNORECASE):
        logger.warning(f"⚠️ Image description references camera. Accepting anyway.")
    ...
    try:
        ...
        if langfuse_public_key and langfuse_secret_key:
            try:
                ...
                logger.info("✅ Langfuse tracking enabled for this generation run.")
            except ImportError:
                logger.warning("⚠️ Langfuse keys found but 'langfuse' package not installed. Skipping tracking.")
            except Exception as e:
                logger.error(f"⚠️ Failed to initialize Langfuse handler: {e}")
